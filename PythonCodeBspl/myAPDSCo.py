# Programm zum Testen der Farbsensorfunktion des Sensors ADPS9960
...
def apdsInit(i2c): #Sensorinitialisierung
    ...
    #print('Device ID read: {}'.format(hex(dev_id)))
...
# Licht-/Farbsensor starten
def enableLightSensor(interrupts=True):
...
# *******************************************************************************
# Licht- und Farbsensor-Messfunktionen
# *******************************************************************************

def readAmbientLight(): # 16 Bit Wert Helligkeit 
    l = read_byte_data(APDS9960_REG_CDATAL)
    h = read_byte_data(APDS9960_REG_CDATAH)
    return l + (h << 8) # Aus zwei 8-Bit ein unsigned 16 Bit Wert machen

def readRedLight(): # 16 Bit Wert Rotanteil
    l = read_byte_data(APDS9960_REG_RDATAL)
    h = read_byte_data(APDS9960_REG_RDATAH)
    return l + (h << 8) # Aus zwei 8-Bit ein unsigned 16 Bit Wert machen
    
def readGreenLight(): # 16 Bit Wert Blauanteil
    l = read_byte_data(APDS9960_REG_GDATAL)
    h = read_byte_data(APDS9960_REG_GDATAH)
    return l + (h << 8) # Aus zwei 8-Bit ein unsigned 16 Bit Wert machen
    
def readBlueLight(): # 16 Bit Wert Gruenanteil
    l = read_byte_data(APDS9960_REG_BDATAL)
    h = read_byte_data(APDS9960_REG_BDATAH)
    return l + (h << 8) # Aus zwei 8-Bit ein unsigned 16 Bit Wert machen
...
def setAmbientLightGain(drive):
    """Sets receiver gain for ambient light sensor (ALS).
        Value    Gain
          0       1x
          1       4x
          2       16x
...
try:
    ...
    print("APDS9960 Helligkeits-/Farbsignal Miniprogramm")
    print("====================================")
    enableLightSensor()
    while True:
        sleep(0.5)
        valA = readAmbientLight()
        valR = readRedLight()
        valG = readGreenLight()
        valB = readBlueLight()
        print("Helligkeit={}, Rot={}, Gruen={}, Blau={}".format(valA, valR, valG, valB))
