...
def sensorRead(blocking=True,rangeonly=False):
    ...
    readResults(rangeonly)
    if not calibrated:
        setupManualCalibration()
        calibrated = True
    if not rangeonly: updateDSS() # unklar ob das zulaessig ist, wenn nur Reichweite gelesen wird
    getRangingData()
    writeReg(SYSTEM__INTERRUPT_CLEAR, 0x01) # sys_interrupt_clear_range
...
def readResults(rangeonly=False):
...
# Unklar ob unbedingt noetig. Wird bei "rangeonly" d.h. maximale Messrate nicht aufgerufen.
def updateDSS():
...
def getRangingData(rangeonly=False):
...
def setROISize(width=16,height=16):
    ...
    writeReg(ROI_CONFIG__USER_ROI_REQUESTED_GLOBAL_XY_SIZE, ((height - 1) << 4) | (width - 1))
...
# lower right.
def setROICenter(spadNumber = 199):
...
try:
    ...
    setDistanceMode(0) # 0=short, 1=medium, 2=long
    setMeasurementTimingBudget(20000) # µs nicht! ms eingeben!
    ...
    while(1):
        ...
        for angle in range(0,13,4):
            ...
            setROICenter(roi_center) # ROI setzen
            sensorReadSingle(blocking=True,rangeonly=True) # Abstandswert auslesen (blocking und nur Reichweite)       
            scan_data.append('{:6.1f}'.format(ranging_data['range_mm']))
        print(scan_data)
